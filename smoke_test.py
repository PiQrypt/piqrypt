#!/usr/bin/env python3
"""
PiQrypt v1.7.0 — Smoke Test / Validation Complète
==================================================
...
def print_summary():
    ...
    if fail: print(f"  {RED}Échecs{RESET}  : {fail}")
    print()
    if fail==0:
        print(f"{GREEN}{BOLD}  ✅  Tous les checks passés — stack v1.7.0 opérationnel{RESET}")
    else:
...
def bloc2_stamp_chain():
    ...
    def chain(n=5):
        ...
        for i in range(1,n):
            ...
            c.append(e); prev=compute_event_hash(e)
        return c
    ...
    def test_sig_tampered():
        ...
        try:
            ok=verify_signature(g,pub); assert ok is False
        except Exception: pass
        return "tampered rejected"

    def test_chain_valid():
        c=chain(5); result=verify_chain(c,identity)
        assert result is True or result
        return f"{len(c)} events"

    def test_chain_tampered():
        c=chain(5); c[2]["payload"]["action"]="TAMPERED"
        try:
            result=verify_chain(c,identity); assert not result
        except Exception: pass
...
def bloc3_memory():
    ...
    priv,pub=generate_keypair(); aid=derive_agent_id(pub)
    ...
    def test_store_many():
        n2=name+"_multi"
        g=stamp_genesis_event(priv,pub,aid,{"action":"g"})
        prev=compute_event_hash(g); store_event_free(g,agent_name=n2)
        for i in range(4):
            e=stamp_event(priv,aid,{"action":f"op_{i}"},previous_hash=prev)
            store_event_free(e,agent_name=n2); prev=compute_event_hash(e)
...
def bloc4_keystore():
    ...
    priv,pub=generate_keypair(); pp="smoke-test-2026"
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        def test_magic():
            with open(kp,"rb") as f: magic=f.read(4)
            assert magic==MAGIC
            return f"magic={magic}"

        def test_wrong_pp():
            try:
                load_encrypted_key(kp,"WRONG"); assert False
            except Exception: pass
            return "rejected"
        ...
        def test_corrupted():
            ...
            try: load_encrypted_key(bad,pp)
            except Exception: pass
            return "handled"

        def test_erase():
            data=bytearray(priv); _secure_erase(data)
...
def bloc6_export_verify():
    ...
    def test_verify_tampered():
        c=bchain(5); c[2]["payload"]["action"]="BAD"
        try:
    ...
    def test_export_json():
        ...
        sz=Path(path).stat().st_size; Path(path).unlink()
...
def bloc7_fork_replay():
    ...
    def test_replay():
        ...
        except ReplayAttackDetected: pass
        return "replay levée ✓"
    ...
    def test_count():
        store=NonceStore()
        for i in range(5): store.check_and_add(aid,f"nonce-cnt-{i}")
...
def bloc8_a2a():
    ...
    priv_a,pub_a=generate_keypair(); a_id=derive_agent_id(pub_a)
    priv_b,pub_b=generate_keypair(); b_id=derive_agent_id(pub_b)
    ...
    def test_verify_tampered():
        ...
        p["agent_id"]=b_id
        try:
            ok=verify_identity_proposal(p); assert not ok
        except Exception: pass
        return "tampered rejected ✓"
...
def bloc9_trust_score():
    ...
    for i in range(4):
        e=stamp_event(priv,aid,{"action":f"op_{i}"},previous_hash=prev)
        chain.append(e); prev=compute_event_hash(e)

    def _score(r):
        return float(r.get("score",r) if isinstance(r,dict) else r)

    def test_I():
        r=compute_I(chain); s=_score(r)
        assert 0.0<=s<=1.0; return f"I={s:.3f}"

    def test_V_t():
        r=compute_V_t(chain); s=_score(r)
        assert 0.0<=s<=1.0; return f"V_t={s:.3f}"

    def test_D_t():
        r=compute_D_t(chain); s=_score(r)
        assert 0.0<=s<=1.0; return f"D_t={s:.3f}"

    def test_F():
        r=compute_F(chain); s=_score(r)
        assert 0.0<=s<=1.0; return f"F={s:.3f}"

    def test_global():
        r=compute_trust_score(aid,events=chain)
        ts=r.get("trust_score",r.get("score")) if isinstance(r,dict) else r
        assert 0.0<=float(ts)<=1.0; return f"TS={float(ts):.3f}"
...
def bloc10_tsi():
    ...
    from aiss.identity import generate_keypair, derive_agent_id
    _,pub=generate_keypair(); aid=derive_agent_id(pub)
    ...
    def test_fresh():
        ...
        assert state in TSI_STATES; return f"state={state}"
    ...
    def test_history():
        h=get_tsi_history(aid); assert isinstance(h,list)
        return f"{len(h)} entries"

    def test_summary():
        s=get_tsi_summary(aid); assert isinstance(s,dict)
...
def bloc11_a2c():
    ...
    def mke(n=5, ts=None):
        evts=[]
        for i in range(n):
            e=stamp_event(priv,aid,{"event_type":"a2a_interaction",
    ...
    def test_empty():
        ...
        assert 0.0<=risk<=1.0; return f"risk={risk:.3f}"
    ...
    def test_concentration():
        ...
        assert isinstance(r,dict); return f"score={r.get('score','?')}"
    ...
    def test_cache():
        invalidate_cache(); invalidate_cache(aid)
        return "OK"
...
def bloc12_vrs():
    ...
    try:
        def test_thresholds():
            for val,exp in [(0.00,"SAFE"),(0.24,"SAFE"),(0.25,"WATCH"),(0.49,"WATCH"),
        ...
        def test_boundaries():
            for val,exp in [(0.2499,"SAFE"),(0.2500,"WATCH"),(0.4999,"WATCH"),
        ...
        if agent_exists(aname): unregister_agent(aname,delete_files=True)
...
def bloc13_vigil():
    ...
    def test_summary():
        ...
        assert status==200, f"HTTP {status}"
        data=json.loads(body); assert isinstance(data,dict)
...
def bloc14_unit_tests():
    ...
    def test_run_all():
        env=os.environ.copy(); env["PIQRYPT_SCRYPT_N"]="16384"
        r=subprocess.run([sys.executable,"tests/run_all.py"],
                         cwd=str(PIQRYPT_ROOT),capture_output=True,text=True,
                         timeout=300,env=env)
...
def main():
    ...
    p=argparse.ArgumentParser(description="PiQrypt v1.7.0 — Smoke Test")
    p.add_argument("-v","--verbose",action="store_true")
    p.add_argument("--stop",action="store_true")
    p.add_argument("--bloc",type=int,choices=range(1,15),metavar="N")
    args=p.parse_args()
    _verbose=args.verbose; _stop_on_first_fail=args.stop

    print(f"\n{BOLD}{CYAN}PiQrypt v1.7.0 — Smoke Test / Validation Complète{RESET}")
    ...
    for num,name,fn in BLOCS:
        if args.bloc and args.bloc!=num: continue
        if not _verbose:
        ...
        try:
            ...
            if not _verbose:
                ...
                total=ok+fail
                if fail==0: print(f"{GREEN}✓ {ok}/{total}{RESET}")
                else: print(f"{RED}✗ {ok}/{total}{RESET}")
        except SystemExit: raise
        except Exception as e:
            if not _verbose: print(f"{RED}✗ ERREUR{RESET}")
            print(f"    {RED}{e}{RESET}")
