...
Architecture v1.8.2 (isolation par agent) :
    ~/.piqrypt/
    └── agents/
        └── <agent_name>/
            ├── events/
...
def _get_dirs(agent_name: Optional[str] = None, session: Any = None):
    ...
    v1.8.2 : utilise agent_registry pour l'isolation par agent.
...
def init_memory_dirs(agent_name: Optional[str] = None, base_dir: Optional[str] = None) -> None:
    """Create directory structure for an agent (v1.8.2) or global (v1.6.0 compat).

    Args:
        agent_name: Nom de l'agent (v1.8.2)
    ...
    if not CONFIG_FILE.exists():
        config = {"version": "1.8.2", "retention_years": 10, "auto_backup": False,
...
def store_event_free(
    ...
    Args:
        ...
        agent_name: Agent name for isolation (v1.8.2). None = fallback "default"
...
def store_event(
    ...
    v1.8.2 : agent_name et session optionnels pour isolation par agent.
...
def get_memory_stats(
    ...
    """Statistiques mémoire d'un agent. agent_name optionnel (v1.8.2)."""
    config = get_config()
