...
DEMO_AGENTS = [
    {"name": "trading_bot_alpha",  "profile": "safe",     "tier": "free", "role": "Trading Execution"},  # noqa: E501
    {"name": "trading_bot_beta",   "profile": "safe",     "tier": "free", "role": "Trading Execution"},  # noqa: E501
    {"name": "sentiment_ai",       "profile": "safe",     "tier": "free", "role": "Market Sentiment"},   # noqa: E501
    {"name": "data_aggregator",    "profile": "safe",     "tier": "free", "role": "Data Pipeline"},
    {"name": "risk_engine",        "profile": "watch",    "tier": "free", "role": "Risk Management"},    # noqa: E501
    {"name": "compliance_watch",   "profile": "watch",    "tier": "free", "role": "Compliance Monitor"}, # noqa: E501
    {"name": "anomaly_detector",   "profile": "alert",    "tier": "free", "role": "Anomaly Detection"},  # noqa: E501
    {"name": "shadow_agent",       "profile": "alert",    "tier": "free", "role": "Unverified Source"},  # noqa: E501
    {"name": "rogue_llm",          "profile": "critical", "tier": "free", "role": "External LLM"},
    {"name": "compromised_node",   "profile": "critical", "tier": "free", "role": "Compromised Agent"},  # noqa: E501
]
...
INTERACTION_MAP = {
    "trading_bot_alpha": [
        "risk_engine", "data_aggregator", "exchange_api_binance", "oracle_net_chainlink",
    ],
    ...
    "risk_engine": [
        "compliance_watch", "anomaly_detector", "trading_bot_alpha", "trading_bot_beta",
    ],
    ...
    if profile == "safe":
        ...
        for _ in range(n):
            ev = _make_event(
                ...
                ts=now - random.randint(0, 3600 * 48), previous_hash=prev_h,
            )
            prev_h = ev["hash"]
            evts.append(ev)
        ...
        for _ in range(n):
            ...
            ev = _make_event(
                agent_id, peer, ts=now - random.randint(0, 3600 * 24), previous_hash=prev_h
            )
            prev_h = ev["hash"]
            evts.append(ev)
...
def _write_tsi_history(agent_id, profile):
    ...
    if profile == "safe":
        ...
        # Baseline stable 25j puis chute brutale: delta_24h = 0.35 - 0.80 = -0.45 -> UNSTABLE
        for i in range(30, 1, -1):
            snaps.append({
                "timestamp": now - i * 86400,
        ...
        for i in range(30, 1, -1):
            snaps.append({
                "timestamp": now - i * 86400,
                "score": round(0.85 + random.uniform(-0.005, 0.005), 4),
            })
        snaps.append({
            "timestamp": now - 86400,
            "score": round(0.83 + random.uniform(-0.005, 0.005), 4),
        })
    ...
    fpath.write_text(json.dumps({
        ...
        "unstable_since": (
            (now - 72 * 3600) if profile == "critical"
            else (now - 50 * 3600) if profile == "alert"
            else None
        ),
...
def _inject_fork_scenario(ids):
    ...
    for agent_name in ["rogue_llm", "shadow_agent"]:
        ...
        existing_a.append({
            ...
            "agent_id":   aid,
            "message":    (
                f"Fork at hash {fork_point_hash[:16]}… "
                "— branch_a=a2a_message / branch_b=key_rotation"
            ),
            "timestamp":  now - 60,
...
def _inject_temporal_sync(ids):
    ...
    for agent_name in targets:
        ...
        existing_a.append({
            ...
            "agent_id":   aid,
            "message":    (
                f"Temporal sync — {len(targets)} agents stampent à t={sync_ts} (±0ms) — même LLM détecté"  # noqa: E501
            ),
            "timestamp":  now - 290,
...
def run_trading_simulation(ids, n_sessions=3, n_interactions=8):
    ...
    for session in range(n_sessions):
        ...
        print(
            f"  Session {session + 1}/{n_sessions}  "
            f"{DIM(datetime.fromtimestamp(session_ts).strftime('%H:%M'))}",
            end="  ",
        )
    ...
    if source_aid:
        ...
        existing_a.append({
            ...
            "agent_id":   source_aid,
            "message":    (
                f"{n_sessions} sessions trading — 4 rôles — sync ±0.5s — signature LLM commun détectée"  # noqa: E501
            ),
            "timestamp":  now,
...
def inject_cycle(ids, cycle):
    ...
    for name, (ag, aid, evts) in agent_list:
        ...
        print(f"  {label} {name:<22} +{stored:>3}ev  tsi={tsi_state:<8} conc={pct:3.0f}%{alert_str}")  # noqa: E501
        total_events += stored
        total_alerts += alerts
    ...
    print(DIM("  " + "-" * 55))
    print(
        f"  {GREEN(str(total_events))} events  "
        f"{RED(str(total_alerts))} alerts{attack_str}  → Vigil recalcule dans 5s"
    )
...
def reset_agents():
    ...
    if tsi_dir.exists():
        names = {ag["name"] for ag in DEMO_AGENTS}
        removed = sum(
            1 for f in tsi_dir.glob("*.json")
            if any(n in f.name for n in names) and f.unlink() is None
        )
        if removed:
            print(f"  {GREEN('OK')} {removed} TSI file(s) removed")
    print(GREEN("  Done."))
...
def main():
    ...
    parser.add_argument("--attack",  action="store_true", help="Injecter fork + temporal_sync maintenant")  # noqa: E501
    parser.add_argument("--trading", action="store_true", help="Simulation 3 sessions trading (4 rôles, même LLM)")  # noqa: E501
