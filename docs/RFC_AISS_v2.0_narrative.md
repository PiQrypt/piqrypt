...
**Residual risk (documented, v1.7.1):** The Vigil HTTP API (`/api/summary`, `/api/alerts`) uses a static `VIGIL_TOKEN` environment variable for authentication. Per-user authentication (OIDC/SSO) is planned for v1.8.3. Until then, Vigil endpoints MUST be treated as internal-only services.
