...
**Residual risk (documented, v1.7.1):** The Vigil HTTP API (`/api/summary`, `/api/alerts`) uses a static `VIGIL_TOKEN` environment variable for authentication. Per-user authentication (OIDC/SSO) is planned for v1.8.0. Until then, Vigil endpoints MUST be treated as internal-only services.
...
| v1.7.0 | Level 2 | 136/143 (7 skipped — external Ollama) |
