...
_PACKAGE_ROOT = _LAUNCHER_DIR.parent if (_LAUNCHER_DIR.name == "cli") else _LAUNCHER_DIR
for _p in [str(_PACKAGE_ROOT), str(_PACKAGE_ROOT / "aiss")]:
...
class StartupCheck:
    ...
    def _check_vigil_files(self):
        vigil_server = _PACKAGE_ROOT / "vigil" / "vigil_server.py"
        vigil_html   = _PACKAGE_ROOT / "vigil" / "vigil_v4_final.html"
    ...
    def _check_trustgate_files(self):
        candidates = [
            _PACKAGE_ROOT / "trustgate" / "trustgate_server.py",
        ]
    ...
    def print_report(self):
        print()
        print(bold("PiQrypt Stack Launcher v1.8.2"))
        print(dim(_SEP * 50))
    ...
    def _build_services(self) -> List[ServiceProcess]:
        ...
        if not self.args.trustgate_only:
            vigil_script = self._resolve_script(
                _PACKAGE_ROOT / "vigil" / "vigil_server.py",
            )
            if vigil_script:
...
def cmd_gen_tokens():
    ...
    env_file = _PACKAGE_ROOT / ".env.piqrypt"
    try:
