...
|-----------|--------|-------------|
| `verify_tsa_token()` checks DER structure only — no full CMS/PKCS7 verification | A crafted TSA token could pass as verified | v1.8.3 |
| Vigil/TrustGate use static `VIGIL_TOKEN` env var | No per-user auth | v1.8.3 OIDC/SSO |
